...
### Model Selection (before spawning)

Before invoking the Task tool, determine the optimal model:

1. **Load rules** — Read `model-routing-rules.yaml` (path from `core-config.yaml` → `modelRouting.rulesFile`)
2. **Build routing context:**
   ```yaml
   taskName: {step task name or step_id}
   agentId: {agent_name from step definition}
   manualOverride: {--model= flag if user provided, else null}
   complexityScore: {from step metadata if annotated, else null}
   estimatedTokens: {from prompt length estimate, else null}
   taskModelHint: {from task metadata model_hint field, else null}
   ```
3. **Evaluate 7-level priority cascade** (see `model-routing-rules.yaml` header)
4. **Log decision:**
   ```
   🧭 Model: {model} (P{priority}) — {reason}
      Cost: {estimatedCost} | Savings: {savings}
   ```

If `modelRouting.enabled` is `false` in `core-config.yaml`, skip model selection and omit the `model` parameter (uses Claude Code default).
...
> **Note:** The `model` parameter is only included when `modelRouting.enabled` is `true`. Valid values: `opus`, `sonnet`, `haiku`.
