...
### VII. Squad First (MUST)

Quando existe um squad especializado para o domínio, ele DEVE ser usado ao invés de agentes genéricos do core.

**Regras:**
- MUST: Tarefas de design DEVEM usar `design-system` squad (não @ux-design-expert genérico)
- MUST: Tarefas de marketing/ads DEVEM usar `media-buy-squad` (não implementar do zero)
- MUST: Tarefas de segurança DEVEM usar `security-squad` (não @dev ou @architect)
- MUST: Clonagem cognitiva DEVE usar `mmos-squad`
- MUST: Scan do PRD/story para keywords de domínio ANTES de iniciar implementação
- MUST NOT: Criar componentes de design, campaigns, ou regras de segurança sem consultar o squad correspondente

**Routing automático:**
```
PRD/Story scan → domain keywords → squad match → squad lead ativa → delega para especialistas
```

**Gate:** Workflow steps que envolvem design/marketing/security DEVEM usar `target_context: "hybrid"` com o squad correspondente.

**Config:** `.aios-core/config/squad-routing-rules.yaml`
